...
if (isset($_GET['username']) && isset($_GET['password']) && isset($_GET['id'])) {
    ...
    $id = $_GET['id'];
    if ($id == '-1') {
        $api_live_channels = $host . ':' . $port . '/player_api.php?username=' . $username . '&password=' . $password . '&action=get_vod_streams';
    } else {
        $api_live_channels = $host . ':' . $port . '/player_api.php?username=' . $username . '&password=' . $password . '&action=get_vod_streams&category_id=' . $id;
    }
    ...
    if ($resp_category !== null) {
        ...
        if ($resp_category !== null) {
            ...
            foreach ($resp_category as $stream) {
                ...
                $m3u_list .= "#EXTINF:-1 type=\"video\" tvg-id=\"{$epg_channel_id}\" tvg-logo=\"{$stream_icon}\" description=\"\",{$name}\n";
